...
def test_keys():
    ...
    keys = mgr.keys(
        series='1.3.6.1.4.1.9328.50.16.251746227724893696781798455517674264022')
    assert 0 == len(keys)
    keys = mgr.keys('abc')
    assert 0 == len(keys)
...
def test_parent():
    ...
    patient = 'RIDER Neuro MRI-5244517593'
    assert mgr.parent(
        series) == '1.3.6.1.4.1.9328.50.16.139212415517744996696223190563412871264'
...
def test_new_child():
    ...
    # Three objects that are not nested: study not in patient and series not
    # in study.
    patient = 'RIDER Neuro MRI-5244517593'
    study = '1.3.6.1.4.1.9328.50.16.168701627691879645008036315574545460110'
    series = '1.3.6.1.4.1.9328.50.16.63380113333602578570923656300898710242'
...
def test_label():
    ...
    assert mgr.label(patient) in [
        'Patient 281949 [RIDER Neuro MRI-3369019796]',
        'Patient 281949 [RIDER Neuro MRI-5244517593]']
...
def test_write():
    ...
    # Change a name in memory, write, clear memory and read again to test this
    # has changed.
...
def test_copy_to_series():
    ...
    assert mgr.value(
        mgr.keys(copy_instances)[0],
        'PatientID') == 'RIDER Neuro MRI-3369019796'
...
def test_move_to_study():
    ...
    assert len(mgr.series(study)) == 1
    assert len(mgr.series(patient)) == 6
    # move to a study of another patient
    series = mgr.move_to_study(patient, study)
    assert len(mgr.instances(study)) == 14
    assert len(mgr.series(study)) == 7
...
def test_set_values():
    ...
    for instance in mgr.instances(patient):
        assert mgr.get_dataset(instance).get_values(
            'AcquisitionTime') != '000000.00'

    mgr.set_values(
        patient, [
            'PatientName', 'SeriesDescription', 'AcquisitionTime'], [
            'Anonymous', 'Desc', '000000.00'])
...
def test_get_values():
    ...
    assert set(values) == set(['074424.477508',
                               '074443.472496',
                               '074615.472480',
                               '074634.479981',
                               '075505.670007',
    ...
    assert set(values[2]) == set(['074424.477508',
                                  '074443.472496',
                                  '074615.472480',
                                  '074634.479981',
                                  '075505.670007',
    ...
    assert set(values[0][1]) == set(
        ['ax 10 flip', 'ax 5 flip', 'sag 3d gre +c'])
    assert set(values[0][2]) == set(['074424.477508',
                                     '074443.472496',
                                     '074615.472480',
                                     '074634.479981',
                                     '075505.670007',
    ...
    assert set(values[1][1]) == set(
        ['ax 10 flip', 'ax 5 flip', 'sag 3d gre +c'])
    assert set(values[1][2]) == set(['073903.155010',
                                     '073921.462495',
                                     '074511.204995',
                                     '074529.432496',
                                     '075138.639989',
...
def test_import_datasets():
    ...
    # Delete one patient, import all source files again and check that nothing
    # has changed
    patient = target_mgr.patients(
        'Database', PatientID='RIDER Neuro MRI-5244517593')
    ...
    patient = target_mgr.patients(
        'Database', PatientID='RIDER Neuro MRI-5244517593')
    target_mgr.delete(patient)
    patient = source_mgr.patients(
        'Database', PatientID='RIDER Neuro MRI-5244517593')
    patient_files = source_mgr.filepaths(patient)
    assert len(target_mgr.instances('Database')) == len(
        source_files) - len(patient_files)
    ...
    patient1 = target_mgr.patients(
        'Database', PatientID='RIDER Neuro MRI-5244517593')
    patient1_files = target_mgr.filepaths(patient1)
    patient2 = source_mgr.patients(
        'Database', PatientID='RIDER Neuro MRI-3369019796')
    patient2_files = source_mgr.filepaths(patient2)
    target_mgr.delete(patient1)
    assert len(target_mgr.instances('Database')) == len(
        source_files) - len(patient1_files)
...
def test_export_datasets():
    ...
    assert len(
        target_mgr.instances('Database')) == len(
        source_mgr.instances(patient))
    assert len(
        target_mgr.series('Database')) == len(
        source_mgr.series(patient))
    assert len(
        target_mgr.studies('Database')) == len(
        source_mgr.studies(patient))
